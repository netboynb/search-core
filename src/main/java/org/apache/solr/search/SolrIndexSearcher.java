...
public class SolrIndexSearcher extends IndexSearcher implements Closeable, SolrInfoMBean {
	...
	public SolrIndexSearcher(SolrCore core, String path, IndexSchema schema, SolrIndexConfig config, String name, DirectoryReader r, boolean closeReader, boolean enableCache,
			boolean reserveDirectory, DirectoryFactory directoryFactory) throws IOException {
	 ...
	 * 
	 * If no cache is present: - use NO_LOAD instead of LAZY_LOAD - use LOAD_AND_BREAK if a single
	 * field is begin retrieved
	 */

	/**
	 * FieldSelector which loads the specified fields, and load all other field lazily.
	 */
	 ...
	 * Retrieve the {@link Document} instance corresponding to the document id.
	 * <p>
	 * Note: The document will have all fields accessible, but if a field filter is provided, only
	 * the provided fields will be loaded (the remainder will be available lazily).
	 */
	@Override
	...
	/**
	 * Takes a list of docs (the doc ids actually) and a set of fields to load, and reads them into
	...
	/**
	 * lookup the docid by the unique key field, and return the id *within* the leaf reader in the
	 * low 32 bits, and the index of the leaf reader in the high 32 bits. -1 is returned if not
	 * found.
	 * 
	 * @lucene.internal
	 */
	 ...
	 * caching, and if so, will compute the DocSet that matches the Query and cache it. If the
	 ...
	 * @param mustCache if true, a best effort will be made to cache this entry. if false,
	 ...
	 * retrieve the answer from the cache if possible. If the answer was not cached, it may have
		...
		if(filterCache != null) {
			...
			if(absAnswer != null) {
				if(positive)
					...
					return getPositiveDocSet(matchAllDocsQuery).andNot(absAnswer);
			}
		}
	...
	/**
	 * Returns documents matching both <code>query</code> and <code>filter</code> and sorted by
	 * <code>sort</code>.
	 * <p>
	 * This method is cache aware and may retrieve <code>filter</code> from the cache or make an
	...
	/**
	 * Returns documents matching both <code>query</code> and the intersection of the
	 * <code>filterList</code>, sorted by <code>sort</code>.
	 * <p>
	 * This method is cache aware and may retrieve <code>filter</code> from the cache or make an
	 * insertion into the cache as a result of this call.
	 * <p>
	...
	/**
	 * getDocList version that uses+populates query and filter caches. In the event of a timeout,
	...
	private void getDocListC(QueryResult qr, QueryCommand cmd) throws IOException {
		...
		if(maxDocRequested < 0 || maxDocRequested > maxDoc())
			maxDocRequested = maxDoc();
		...
		//you should check cmd.getFilterCollectors()=? null 
		if(queryResultCache != null && cmd.getFilter() == null && cmd.getFilterCollectors() == null && (flags & (NO_CHECK_QCACHE | NO_SET_QCACHE)) != ((NO_CHECK_QCACHE | NO_SET_QCACHE))) {
	...
	private void getDocListNC(QueryResult qr, QueryCommand cmd) throws IOException {
		...
		if(lastDocRequested <= 0) {
			...
			// add collect filter
			List<FilterableCollector> filterCollectors = cmd.getFilterCollectors();
			if(filterCollectors != null && filterCollectors.size() > 0) {
				org.apache.solr.search.function.cf.FilterCollector filterCollector = new org.apache.solr.search.function.cf.FilterCollector(collector, filterCollectors);
	...
	/**
	 * Returns documents matching both <code>query</code> and <code>filter</code> and sorted by
	 * <code>sort</code>. FUTURE: The returned DocList may be retrieved from a cache.
	 * 
	 * @param filter may be null
	 * @param lsort criteria by which to sort (if null, query relevance is used)
	 * @param offset offset into the list of documents to return
	...
	/**
	 * Returns documents matching both <code>query</code> and <code>filter</code> and sorted by
	 * <code>sort</code>. Also returns the complete set of documents matching <code>query</code> and
	 * <code>filter</code> (regardless of <code>offset</code> and <code>len</code>).
	 * <p>
	 * This method is cache aware and may retrieve <code>filter</code> from the cache or make an
	 * insertion into the cache as a result of this call.
	 * <p>
	...
	/**
	 * Returns documents matching both <code>query</code> and <code>filter</code> and sorted by
	 * <code>sort</code>. Also returns the compete set of documents matching <code>query</code> and
	 * <code>filter</code> (regardless of <code>offset</code> and <code>len</code>).
	 * <p>
	 * This method is cache aware and may retrieve <code>filter</code> from the cache or make an
	 * insertion into the cache as a result of this call.
	...
	/**
	 * Returns documents matching both <code>query</code> and the intersection of
	 * <code>filterList</code>, sorted by <code>sort</code>. Also returns the compete set of
	 * documents matching <code>query</code> and <code>filter</code> (regardless of
	...
	/**
	 * Takes a list of docs (the doc ids actually), and returns an array of Documents containing all
	...
	public NamedList<Object> getStatistics() {
		...
		if(registerTime != 0)
			lst.add("registeredAt", new Date(registerTime));
	...
	public static class QueryCommand {
		...
		public QueryCommand setFilterCollectors(List<FilterableCollector> filterCollectors) {
			this.filterCollectors = filterCollectors;
			return this;
		}

		public QueryCommand addFilterCollectors(FilterableCollector filterCollector) {
			if(filterCollectors == null) {
				filterCollectors = new ArrayList<FilterableCollector>(3);
			}
			filterCollectors.add(filterCollector);
			return this;
		}

		public void removeFilterCollector(FilterableCollector filterCollector) {
			if(filterCollectors != null) {
				filterCollectors.remove(filterCollector);
			}
		}
