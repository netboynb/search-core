...
public class CoreAdminHandler extends RequestHandlerBase {
	...
	/**
	 * after the merge ,we should delete the realtime-tempCore's index
	 */
	public boolean handleMergeAction(SolrQueryRequest req, SolrQueryResponse rsp) throws IOException {
		SolrParams params = req.getParams();
	 ...
	 * <p/>
	 * This method could be overridden by derived classes to handle custom actions. <br>
	...
	protected boolean handleCustomAction(SolrQueryRequest req, SolrQueryResponse rsp) {
		throw new SolrException(SolrException.ErrorCode.BAD_REQUEST, "Unsupported operation: " + req.getParams().get(CoreAdminParams.ACTION));
	}
	...
	protected boolean handleSwapAction(SolrQueryRequest req, SolrQueryResponse rsp) {
		...
		log.info("swap [{}]-core to [{}]-core", cname, other);
	...
	protected void handleWaitForStateAction(SolrQueryRequest req, SolrQueryResponse rsp) throws IOException, InterruptedException, KeeperException {
		...
		while(true) {
			...
			try {
				...
				if(retry++ == 120) {
					throw new SolrException(ErrorCode.BAD_REQUEST, "I was asked to wait on state " + waitForState + " for " + nodeName + " but I still do not see the requested state. I see state: "
							+ state + " live:" + live);
				}
