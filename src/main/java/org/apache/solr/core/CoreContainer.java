...
public class CoreContainer {
	...
	protected final Map<String, Exception> coreInitFailures = Collections.synchronizedMap(new LinkedHashMap<String, Exception>());
	...
	private String[] actualCores = { "big", "realtime", "realtime-temp" };
	{
	...
	protected void initZooKeeper(String zkHost, int zkClientTimeout) {
		...
		if(zookeeperHost != null) {
			...
			try {
				...
				zkController = new ZkController(this, zookeeperHost, zkClientTimeout, zkClientConnectTimeout, host, hostPort, hostContext, leaderVoteWait, distribUpdateConnTimeout,
						distribUpdateSoTimeout, new CurrentCoreDescriptorProvider() {
	...
	public void load(String dir, InputSource cfgis) {
		...
		String actualCoresStr = cfg.get("solr/cores/@actualCores", null);
		if(actualCoresStr != null) {
			actualCores = actualCoresStr.split(",");
		}
		...
		try {
			...
			for(int i = 0; i < nodes.getLength(); i++) {
				...
				try {
					...
					if(opt != null) {
						p.setTransient(("true".equalsIgnoreCase(opt) || "on".equalsIgnoreCase(opt)) ? true : false);
					}
	...
	/**
	 * Registers a SolrCore descriptor in the registry using the specified name. If
	 * returnPrevNotClosed==false, the old core, if different, is closed. if true, it is returned
	 * w/o closing the core
	 * 
	...
	protected SolrCore registerCore(Map<String, SolrCore> whichCores, String name, SolrCore core, boolean returnPrevNotClosed) {
		...
		synchronized(whichCores) {
			...
			/*
			 * set both the name of the descriptor and the name of the core, since the descriptors
			 * name is used for persisting.
			 */
			core.setName(name);
	...
	/**
	 * Registers a SolrCore descriptor in the registry using the core's name. If returnPrev==false,
	 * the old core, if different, is closed.
	 * 
	 ...
	 * <p>
	 * While the Map returned by this method is immutable and will not change once returned to the
	 * client, the source data used to generate this Map can be changed as various SolrCore
	...
	/**
	 * Recreates a SolrCore. While the new core is loading, requests will continue to be dispatched
	 * to and processed by the old core
	 * 
		...
		try {
			...
			if(zkController == null) {
				...
				try {
					...
					solrLoader = new ZkSolrResourceLoader(instanceDir.getAbsolutePath(), zkConfigName, libLoader, getCoreProps(instanceDir.getAbsolutePath(), cd.getPropertiesName(),
							cd.getCoreProperties()), zkController);
				} catch(KeeperException e) {
					log.error("", e);
					throw new ZooKeeperException(SolrException.ErrorCode.SERVER_ERROR, "", e);
				} catch(InterruptedException e) {
	...
	/**
	 * Sets the alternate path for multicore handling: This is used in case there is a registered
	...
	public void persistFile(File file) {
		...
		if(libDir != null)
			rootSolrAttribs.put("sharedLib", libDir);
	...
	private SolrConfig getSolrConfigFromZk(String zkConfigName, String solrConfigFileName, SolrResourceLoader resourceLoader) {
		...
		try {
			...
			cfg = solrConfigFileName == null ? new SolrConfig(resourceLoader, SolrConfig.DEFAULT_CONF_FILE, is) : new SolrConfig(resourceLoader, solrConfigFileName, is);
		} catch(Exception e) {
			throw new SolrException(ErrorCode.SERVER_ERROR, "getSolrConfigFromZK failed for " + zkConfigName + " " + solrConfigFileName, e);
		}
	...
	private static final String DEF_SOLR_XML = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n" + "<solr persistent=\"false\">\n" + "  <cores adminPath=\"/admin/cores\" defaultCoreName=\""
			+ DEFAULT_DEFAULT_CORE_NAME + "\"" + " host=\"${host:}\" hostPort=\"${hostPort:}\" hostContext=\"${hostContext:}\" zkClientTimeout=\"${zkClientTimeout:15000}\"" + ">\n"
			+ "    <core name=\"" + DEFAULT_DEFAULT_CORE_NAME + "\" shard=\"${shard:}\" collection=\"${collection:}\" instanceDir=\"collection1\" />\n" + "  </cores>\n" + "</solr>";
